...
`Phi(x)` is reused by both terms, which guarantees the same selected frozen
reference denoiser, candidate prompt condition, sigma schedule, and exact noise
table. This is the original merged baseline unless rolling reference updates
are enabled. Same-prompt references use the candidate prompt; diverse
references use their own prompts,
...
## Rolling Frozen Reference Model

NFT can opt into a rolling frozen reference without changing the default
training path:

```python
config.creativity.reference_model_update.enabled = True
config.creativity.reference_model_update.interval_epochs = 5
config.creativity.reference_model_update.min_remaining_epochs = 3
```

The CFG-distillation LoRA remains merged into the transformer base. A separate
frozen PEFT adapter named `reference` starts with zero weights, making the
initial reference exactly the merged baseline. At each eligible boundary the
current trainable `default` LoRA is copied into this adapter. The snapshot is
not merged and is excluded from optimization.

For 20 training epochs, epochs 1-5 use the original baseline, epochs 6-10 use
the snapshot after epoch 5, epochs 11-15 use the snapshot after epoch 10, and
epochs 16-20 use the snapshot after epoch 15. A boundary update is skipped when
fewer than `min_remaining_epochs` remain, so no unused terminal snapshot is
created.

The rolling adapter replaces the original standalone baseline in every
training reference role:

- The NFT KL penalty compares the trainable prediction with the frozen rolling
  reference prediction.
- Same-prompt and diverse reference images are sampled from the rolling model.
- Candidate and reference equation-16 features use the rolling denoiser for
  IEM-squared, `-g`, and `-log p - g`.

The `old` NFT policy and trainable `default` policy are unchanged. Fixed
validation and creative-probe baselines remain the original baseline so visual
comparisons stay stable across epochs and across experiments.

A compatible RAM reference cache can be read only during zero-based epochs
0-4. From epoch 5 onward, references and statistics are recomputed because the
model identity has changed. Updating the snapshot also clears the diverse
reference reservoir, so it cannot mix endpoints generated by different
snapshots.

NFT checkpoints save the frozen `reference` adapter and its source epoch
alongside `default`, `old`, optimizer, RNG, and reward state. Resume rejects
a missing rolling adapter, a rolling checkpoint loaded with the mode disabled,
or a snapshot epoch inconsistent with the next epoch. W&B reports
`reference_model/epoch` and `reference_model/updated` while the mode is
enabled.
